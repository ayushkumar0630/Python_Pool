...
def sender_round1(rnd_bytes=os.urandom):
    ...
    a = uint256_from_str(rnd_bytes(32)) % order
    a = Fp(a)
    A = a.n * G
    print type(A)
    return a, A
...
def receiver_round2(c, A, rnd_bytes=os.urandom, RO=sha2):
    ...
    # TODO: Your code goes here
    b = uint256_from_str(rnd_bytes(32)) % order
    b = Fp(b)
    if c == 0:
      B = b.n * G
    else:
...
def receiver_round3(c, kR, e0, e1):
    ...
    """
    assert type(kR) is str and len(kR) == 32
    assert c in (0,1)
    # TODO: your code goes here
    if c == 0:
      return decrypt(kR, e0)
    else:
      return decrypt(kR, e1)
...
def strxor(a,b):
    ...
    assert type(a) is type(b) is str
